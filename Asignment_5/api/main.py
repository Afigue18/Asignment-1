...
def read_one_recipe(recipe_id: int, db: Session = Depends(get_db)):
    result = recipes.read_one(db, recipe_id)
    if result is None:
...
@app.get("/order-details/{detail_id}", response_model=schemas.OrderDetail, tags=["Order Details"])
def read_one_order_detail(detail_id: int, db: Session = Depends(get_db)):
    result = order_details.read_one(db, detail_id)
    if result is None:
        raise HTTPException(status_code=404, detail="Order detail not found")
    return result


@app.put("/order-details/{detail_id}", response_model=schemas.OrderDetail, tags=["Order Details"])
...
@app.delete("/order-details/{detail_id}", tags=["Order Details"])
